...
$("#new-password").on("input", function () {
    ...
    if (newPasswordLength > 0){
        shieldXIcon.toggle(newPasswordLength < 5);
        shieldExclamationIcon.toggle(newPasswordLength >= 5 && newPasswordLength < 6);
...
$("#new-password, #confirm-password").on("input focus", function () {
    ...
    if (confirmPasswordLength === 0 || newPasswordLength < 5 ){
        changeButton.prop('disabled', true);
    } else {
        changeButton.prop('disabled',false)
    }
